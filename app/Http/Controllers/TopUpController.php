...
class TopUpController extends Controller
{
    function handle(Request $request){
        ...
        $_CONFIG['TMN'][50]['point'] = 60;
        $_CONFIG['TMN'][90]['point'] = 100;
        $_CONFIG['TMN'][150]['point'] = 160;
        $_CONFIG['TMN'][300]['point'] = 320;
        $_CONFIG['TMN'][500]['point'] = 550;
        $_CONFIG['TMN'][1000]['point'] = 1100;
        if($_SERVER['REMOTE_ADDR'] == '203.146.127.115' && isset($request['request'])){
            file_put_contents('/var/www/wowlike/public/a.txt',$request['request'].PHP_EOL.PHP_EOL.PHP_EOL , FILE_APPEND | LOCK_EX);
        	$aes = new Crypt_AES();
        	$aes->setKey(API_PASSKEY);
        	$request['request'] = base64_decode(strtr($request['request'], '-_,', '+/='));
        	$request['request'] = $aes->decrypt($request['request']);
        	if($request['request'] != false)
        	{
        		parse_str($request['request'],$request);
        		$request['Ref1'] = base64_decode($request['Ref1']);
        		$result = DB::collection('user')->where('_id',$request['Ref1'])->first();
        		if($result){
                  ...
                  if(DB::collection('user')->where('_id',$request['Ref1'])->update(['money'=>($result['money'] + $_CONFIG['TMN'][$request['cardcard_amount']]['point'])])){
                      DB::collection('log_topup')->insert(['fbid'=>$result['fbid'],'name'=>base64_decode($request['Ref2']),'card_password'=>$request['cashcard_password'],'amount'=>$request['cardcard_amount'],'point'=>$_CONFIG['TMN'][$request['cardcard_amount']]['point'],'created_at'=>date('c',time())]);
                  }
                  //echo $success.$_CONFIG['TMN'][$request['cardcard_amount']]['point'];
        		  //$page->sqlite_query('insert into charge_history (username,point,info,pin,ref1,ref2,ref3) VALUES ("'.$request['Ref1'].'","'.$request['cardcard_amount'].'","'.SQLite3::escapeString($request).'","'.$request['cashcard_password'].'","'.$request['Ref1'].'","'.base64_decode($request['Ref2']).'","'.base64_decode($request['Ref3']).'")');
